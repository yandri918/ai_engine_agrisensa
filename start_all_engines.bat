...
start "AgriSensa MLOps API (8000)" cmd /k "cd 03_mlops_inference_api && %PY_CMD% -m uvicorn main:app --host 0.0.0.0 --port 8000 --reload"

echo [2/4] Starting AgriSensa Advanced AI & MCP Engine (Port 8001)...
start "AgriSensa AI Engine (8001)" cmd /k "cd 02_ai_reasoning_mcp && %PY_CMD% -m uvicorn api.main:app --host 0.0.0.0 --port 8001 --reload"

echo [3/4] Starting AgriSensa Streamlit Dashboard (Port 8501)...
start "AgriSensa Streamlit (8501)" cmd /k "cd 05_frontend_clients\agrisensa_streamlit_dashboard && %PY_CMD% -m streamlit run Home.py"
